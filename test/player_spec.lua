...
describe("Player", function()
  ...
  describe("#use_tool", pending("todo"))
  ...
  describe("#update", function()
    it("updates the player's state", function()
      Player.change_state(player, "eating")
      Player.update(player)
      assert.are.same("eating", player.state.current_state)
    end)
  end)
  ...
  describe("#give_energy", function()
    it("can give the player energy", function()
      player.energy = 1
      Player.give_energy(player, 1)
      assert.are.same(2, player.energy)
    end)
  end)
  ...
  describe("#set_energy", function()
    it("can set the energy", function()
      Player.set_energy(player, 1)
      assert.are.same(1, player.energy)
      Player.set_energy(player, player.max_energy)
      assert.are.same(player.max_energy, player.energy)
    end)

    it("can not set energy more than their max energy", function()
      Player.set_energy(player, player.max_energy + 1)
      assert.are.same(player.max_energy, player.energy)
    end)

    it("can not set energy below 0", function()
      Player.set_energy(player, -1)
      assert.are.same(0, player.energy)
    end)
  end)

  describe("#upgrade_max_energy", function()
    it("can increase the player's max_energy", function()
      local new_max = player.max_energy + 10
      Player.upgrade_max_energy(player, new_max)
      assert.are.same(player.max_energy, new_max)
    end)

    it("can not set max_energy below 1", function()
      Player.upgrade_max_energy(player, -1)
      assert.are.same(1, player.max_energy)
    end)
  end)

  describe("#change_state", function()
    it("sets the next state", function()
      Player.change_state(player, "eating breakfast")
      assert.are.same(player.state.next_state, "eating breakfast")
    end)
  end)
end)
